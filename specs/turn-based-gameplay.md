...
2. Stroke extension (`mousemove`/`touchmove`, both `preventDefault`): each point is appended to `currentLine`. On every extension the client runs `checkCollisions`: crossing any existing line (with a 15 px safe zone around the start number), self-intersection, or bringing the stroke head within 20 px of any number that is neither the current nor the next one. On a hit, the client emits `game_over { roomCode, reason: "Cruzó una línea", lastLine }`, marks itself lost locally, and stops (see the game-end spec for the server side). Otherwise, if the new head is within 20 px of the number with `value === currentNumber + 1`, the stroke **auto-completes**: the shared `finishStroke()` routine snaps the endpoint to that number's center, runs the final `checkCollisions`, and emits `submit_move` (clean) or `game_over` (hit), clears the stroke, sets `isMyTurn = false`, and redraws — pointer events after that are inert (see the auto-complete-stroke spec).
3. Stroke release (`mouseup` on `window`, or `touchend`): if `currentLine` exists and the game is not over, the client finds the number with `value === currentNumber + 1`:
   - Release within 20 px → the same shared `finishStroke()` completion as the auto-complete path (snap to center → final check → submit or lose).
   - Release anywhere else (target never reached) → the line is discarded (`currentLine = null`), nothing is emitted.
